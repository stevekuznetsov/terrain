...
def generate_support_for_surface(config, debug_config, surface, logger):
    ...
    init_elemental_neighborhood_constraint = elemental_neighborhood_constraint(
        i, j, k, indices, feature_radius_pixels, value_getter, value_producer
    )
...
def variable_getter(v):
    """
    This getter returns the variable inside of a Pyomo model.
    """
    return v
...
def value_getter(v):
    """
    This getter returns the value of a Pyomo model variable.
    """
    return v.value
...
def concrete_model(shape, surface, feature_radius_pixels, self_supporting_angle_degrees,
    ...
    model.elemental_neighborhood_constraint = pyo.Constraint(
        ...
        rule=elemental_neighborhood_constraint(
            I_e, J_e, K_e, surface, feature_radius_pixels, variable_getter, equality_constraint_producer
        )
    )
